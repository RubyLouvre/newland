$.define("cache_page","../more/tidy_css,../more/tidy_html,../more/tidy_js",function(tidy_css,tidy_html,tidy_js){
    return function( flow ){
      ...
      //  var matchOne = /<pre class="brush:(\w+)(?:[^"]+)">((?:.|\n|\r)+?)<\/pre>/i
        var matchAll = /<pre(?:.)+?class\s*=\s*(['"])\s*prettyprint([^"']*?)\1\s*>(?:.|\n|\r)+?<\/pre>/gi
        var matchOne = /<pre class="prettyprint([^"]*?)">((?:.|\n|\r)+?)<\/pre>/i
        flow.bind('cache_page', function( html, url ){
            ...
            html = html.replace(matchAll, function(ss){
                ...
                var type = (match[1]||"").trim() || "javascript";
                var context = match[2]
                switch(type){
                    case "javascript":
                    case "js":
                ...
                buffer.push( context )
                return '<pre class="prettyprint '+type+'">mass_mass</pre>'
            });
            html = tidy_html(html);
            var index = 0
