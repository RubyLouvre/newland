...
(function (window, undefined) {
    ...
    //
    //解析引擎，最主要的用法是new(less.Parser)().parse，是没有传参，env可能是为了向前兼容
    // 由于方法间的连联太频繁,从原型上调来调去太低效,改成如下结构
            ...
            parse: function (str, callback) {
                ...
                chunks = (function (chunks) {
                    ...
                    for (var i = 0, c, cc; i < input.length; i++) {
                        ...
                        switch (c) {
                            case '{': if (! inParam) {
                                level ++;        chunk.push(c);                           break
                            }
                            case '}': if (! inParam) {
                                level --;        chunk.push(c); chunks[++j] = chunk = []; break
                            }
                            case '(': if (! inParam) {
                                inParam = true;  chunk.push(c);                           break
                            }
                            case ')': if (  inParam) {
                                inParam = false; chunk.push(c);                           break
                            }
