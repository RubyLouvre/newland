define("position",["$css","$node"],function() {
    ...
    if(  $.supports.fixed === void 0 ){
        (function(){
            ...
            $.support.fixed = test.offsetTop !== control.offsetTop;
    ...
    isPinFixed = false,
    IEFixed = $.supports.fixed == false;

    // 将目标元素相对于基准元素进行定位
    // 这是 Position 的基础方法，接收两个参数，分别描述了目标元素和基准元素的定位点
    Position = {
        pin: function(pinObject, baseObject) {
            ...
            if (pinElement.css('position') !== 'fixed' || IEFixed) {
