...
mass.define("class", "lang",function(){
    mass.log("已加载class模块")
    var
    P = "prototype",  C = "constructor", I = "@init",S = "_super",
    unextend = mass.oneObject([S,P, 'extend', 'implement','_class']),
    exclusive = new RegExp([S,I,C].join("|")),ron = /on([A-Z][A-Za-z]+)/,
    classOne = mass.oneObject('Object,Array,Function');
    function expand(klass,props){
        'extend,implement'.replace(mass.rword, function(name){
            var modules = props[name];
            if(classOne[mass.type(modules)]){
    ...
    function setOptions(){
        var options = this.options = mass.Object.merge.apply(this.options || {}, arguments),key,match
    ...
    mass.factory = function(obj){
        ...
        mass.mix(klass,mass["@class"]).inherit(parent, init);//添加更多类方法
